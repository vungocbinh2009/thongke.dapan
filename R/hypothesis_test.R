...
#' @import thongke
#' @export
answer_test_mean_norm <- function(data) {
...
#' @import xtable
#' @export
answer_test_goodness_of_fit <- function(data, statement) {
  file_name <- get_file_path("template", "hypothesis_test", "test_goodness_of_fit.mustache")
  params <- data$params
  result <- data$result
  col_names <- seq_along(params$actual)
...
#' @import thongke
#' @export
answer_test_2_mean_norm <- function(data) {
...
#' @import xtable
#' @export
answer_test_independent <- function(data, statement) {
  file_name <- get_file_path("template", "hypothesis_test", "test_independent.mustache")
  params <- data$params
  result <- data$result
  row_names <- c(seq_len(nrow(params$matrix)), "Tổng")
  col_names <- c(seq_len(ncol(params$matrix)), "Tổng")
